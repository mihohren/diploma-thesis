...
Class signature := {
    ...
    pred_arr : PredS -> nat;
    IndPredS : Type;
    indpred_arr : IndPredS -> nat;
    }.

Arguments FuncS {Σ} : rename.
Arguments PredS {Σ} : rename.
Arguments IndPredS {Σ} : rename.
...
Section term.
  ...
  Definition vec_id {A : Type} {f : A -> A} (Hid : forall x, f x = x) :
    forall {n} (v : vec n A), V.map f v = v.
  ...
  Definition vec_ext {A B} {f g : A -> B} :
    (forall x, f x = g x) -> forall {n} (v : vec n A),  V.map f v = V.map g v.
  ...
  Definition vec_comp {A B C} {f: A -> B} {g: B -> C} {h} :
    ...
    forall {n} (v : vec n A), V.map g (V.map f v) = V.map h v.
  Proof.
    intros Heq n; induction v.
    - reflexivity.
...
Global Instance Subst_term (Σ : signature)  : Subst1 (fin -> term ) (term ) (term ) := @subst_term Σ   .

Global Instance Subst_formula (Σ : signature)  : Subst1 (fin -> term ) (formula ) (formula ) := @subst_formula Σ  .

Global Instance VarInstance_term (Σ : signature) : Var (fin) (term ) := @var_term Σ .
...
Global Instance Up_term_term (Σ : signature)  : Up_term (_) (_) := @up_term_term Σ  .
