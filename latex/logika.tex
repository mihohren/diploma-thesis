...
Preostaje definicija \textbf{pravila za indukciju}, odnosno pravila za induktivne predikatne simbole.
...
Neka je \(P_{j}\) proizvoljni induktivan predikatni simbol za koji definiramo lijevo pravilo indukcije.
...
\begin{example}
  ...
  \end{prooftree}
  \noindent gdje je G induktivna hipoteza vezana uz predikatni simbol \(\mathit{Nat}\),
  a \(x\) je varijabla koja se ne javlja u \(\Gamma\), \(\Delta\) ili \(\mathit{Nat}(t)\).
\end{example}

\begin{example}
  Tvrdnja \enquote{svaki prirodan broj je paran ili neparan} je dokaziva u sustavu \(\mathit{LKID}_{\mathit{PA}}\).
  \misao{TODO: napraviti prooftree ovog dokaza.}
\begin{minted}{coq}
...
\begin{example}
  TODO Induktivno pravilo za Even.
\end{example}

Konačno, neka je \(pr\) proizvoljna produkcija za koju definiramo desno pravilo indukcije.
...
Valja primijetiti suptilnu razliku između pravila za indukciju;
