...
U ovom poglavlju dajemo pregled visoke razine programskog sustava Coq.
...
Skupni način rada pokreće se naredbom \texttt{coqc}, a korisniku omogućuje provjeru tipova i prevođenje izvornih datoteka u strojno čitljive formate.
...
\item[Vernacular] (\enquote{govorni jezik}) je jezik naredbi kojima korisnik komunicira sa sustavom (i u interaktivnom i u skupnom načinu rada);
  svaka Coqova skripta (datoteka s nastavkom \texttt{.v})\, je niz naredbi.
  Često korištene naredbe su \texttt{Check}, \texttt{Definition}, \texttt{Inductive}, \texttt{Fixpoint} i \texttt{Lemma}.
...
\noindent Pogledajmo primjer za ilustraciju odnosa ta tri jezika.
\begin{minted}{coq}
...
izraz \verb|example_lemma: 1+1=2| dio je Galline,
...
Prve inačice Coqa implementirale su samo račun konstrukcija~\cite{coc} koji je
...
Naredbom \texttt{Check} možemo provjeriti tip nekog terma ili doznati da termu Coq ne može dodijeliti tip.
Dalje u radu pod \enquote{term} mislimo na dobro formirane terme, odnosno na one kojima Coq može dodijeliti tip.
...
Važno je napomenuti da izrazi koji počinju s \texttt{match t} (\engl{pattern matching}),
gdje je \texttt{t} stanovnik tipa \texttt{T}, moraju imati po jednu granu za svaki konstruktor tipa \texttt{T}.\footnote{U sprezi s uvjetom strukturalne rekurzije, ovime je osigurana totalnost svake funkcije.}
...
(* Definition plus := fix plus (n m : nat) {struct n} : nat := *)
match n with
| O => m
| S n' => S (plus n' m)
end.
...
Standardni primjer koinduktivnog tipa je beskonačna lista.
\begin{minted}{coq}
...
Kada bi takav \texttt{Type} postojao, tada bi vrijedilo \verb|Type:Type|, što može dovesti do paradoksa samoreferenciranja.\footnote{U naivnoj logici to je Epimenidov paradoks (\enquote{Ova je rečenica lažna.}), a u naivnoj teoriji skupova to je Russellov paradoks, u naivnoj teoriji tipova to je Girardov paradoks.}
...
Ovaj stil kvantifikacije omogućuje nam definiciju proizvoljnih propozicijskih veznika.\footnote{U praksi su veznici induktivno definirani.}
...
Kada bi sorta \coqprop{} bila predikativna,
ne bismo mogli definirati predikat \texttt{not:~Prop~\(\rightarrow\)~Prop}.
...
\end{displayquote}
\noindent Time se dokazivanje svodi na programiranje. Pogledi na dvije strane ovog \enquote{novčića} mogu se vidjeti u tablici~\ref{tab:curryhoward}.
\begin{table}[!hbt]
  \centering
...
Ako je pak \(n = n^{\prime} + 1\) za neki \(n^{\prime}\), tada rekurzivno konstruiramo dokaz za \(P(n^{\prime})\), a tražena tvrdnja slijedi primjenom \textit{koraka} indukcije na rekurzivno konstruirani dokaz.
\begin{minted}{coq}
Definition nat_ind (P : nat -> Prop)
                   (baza : P 0)
                   (korak : forall m, P m -> P (S m))
  : forall n, P n :=
  fix F (n : nat) : P n :=
    match n with
    | O => baza
