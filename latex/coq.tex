...
\begin{description}
\item[Vernacular] \misao{\textit{vernacular} znači \enquote{govorni jezik}} je jezik naredbi kojima korisnik komunicira sa sustavom (i u interaktivnom i u skupnom načinu rada);
...
Dalje u radu pod \enquote{term} mislimo na dobro formirane terme, odnosno na one terme kojima se može dodijeliti tip.
...
\[
  \textrm{\enquote{Induktivni tipovi su domene rekurzivnih funkcija, a koinduktivni tipovi su kodomene korekurzivnih funkcija}.}
\]
...
Primjer jednog takvog tipa je \enquote{lista duljine \(n\)}, gdje je \(n\) neki prirodan broj, a čiji su stanovnici \(n\)-torke.
Općeniti tip tada glasi \enquote{za svaki \(n\), lista duljine \(n\)} --- on ovisi o stanovniku drugog tipa (u ovom slučaju, tipa \texttt{nat}).
...
Umjesto jedne \enquote{velike} sorte \texttt{Type}, u Coqu postoji niz monotono rastućih sorti \coqtype{n} za sve prirodne brojeve \(n\),
...
\misao{NB: Tu je prije pisalo da su stanovnici od \texttt{Set} programi, no to nije tako. Stanovnici sorte \texttt{Set} su mali tipovi (ne znam kako ih drugačije nazvati), a tek onda su stanovnici malih tipova programi.} 
...
\[
  \textrm{\enquote{Propozicije su tipovi, dokazi su programi.}}
\]
...
\misao{NB: uvjet pozitivnosti mora vrijediti i u pozitivnim i u negativnim koinduktivnim tipovima.}
...
\misao{Ipak neću spomenuti razlike između pozitivnih i negativnih koinduktivnih tipova.
...
Sam postupak dokazivanja smatra se \enquote{implementacijskim detaljom}.
